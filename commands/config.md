...
5. **Codemap auto-refresh** (`hooks.codemap_refresh`) — When ON, two hooks (`session-start-codemap-check.sh`, `stop-codemap-refresh.sh`) keep registered codemaps in sync with their source files. At session start, every `.manifest.json` under `vault/` is scanned for drift (manual edits since last session, files added/removed by `git pull`); any stale entries are regenerated by a background API worker. At session end, files Claude edited this session are checked against every codemap manifest and refreshed similarly. Requires a codemap registered via `/sentinel-codemap register`, `ANTHROPIC_API_KEY` in the environment, and `python3` on `PATH`. Costs ~$0.0001 per file refreshed on `claude-haiku-4-5`. Off by default — enable when you have at least one codemap registered.

6. **Session summary** (stop-session-summary.sh) — When a session ends, save a summary of what was done to `vault/session-recovery/`. Useful for picking up where you left off in a new session.

7. **Vault search on prompt** (prompt-vault-search.sh) — Before processing each prompt, search `vault/gotchas/` and `vault/investigations/` for relevant entries. Adds latency but prevents repeating known mistakes.

8. **Design review reminder** (post-tool-design-check.sh) — After editing `.tsx` or `.css` files, remind to run design review. Only useful for frontend projects.
